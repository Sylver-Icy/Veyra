class BattleManager:
    ...
    def resolve_round(self):
        ...
        if self.arena:
            note = self.arena.on_round_start(self)
            if note:
        ...
        if player1.hp <= 0 and player2.hp <= 0:
            return _r(f"Both {player1.name} and {player2.name} have fallen. It's a tie!")
        elif player1.hp <= 0:
            return _r(f"{player1.name} has fallen. {player2.name} wins!")
        elif player2.hp <= 0:
            return _r(f"{player2.name} has fallen. {player1.name} wins!")
        ...
        if player1.current_stance == 'attack' and player2.current_stance == 'attack':
            ...
            if player1.speed > player2.speed:  # p1 attacks first
                ...
                if player2.hp <= 0:
                    return _r(f"{player1.name} killed {player2.name} before they could attack")

                p2_dmg = player2.deal_dmg(player1)
                return _r(f"{player1.name} and {player2.name} both chose to attack. Since {player1.name} is faster "
            ...
            if player2.speed > player1.speed:  # p2 attacks first
                p2_dmg = player2.deal_dmg(player1)
                if player1.hp <= 0:
                    return _r(f"{player2.name} killed {player1.name} before they could attack")

                p1_dmg = player1.deal_dmg(player2)
                return _r(f"{player1.name} and {player2.name} both chose to attack. Since {player2.name} is faster "
            ...
            return _r(f"coz of same speed both attacked at same time {player1.name} dealt {p1_dmg} dmg and "
        ...
        if player1.current_stance == 'attack' and player2.current_stance == 'block':
            ...
            if block_result['status'] == 'fullsuccess':
                return _r(f"{player2.name} used his hammer to block the incoming attack taking no dmg at all while gaining {block_result['defense_buff']} defense")
        ...
        if player1.current_stance == 'attack' and player2.current_stance == 'counter':
            ...
            return _r(f"{player1.name} attacked but {player2.name} countered dealing {counter_dmg} damage back.")
        ...
        if player1.current_stance == 'block' and player2.current_stance == 'attack':
            ...
            if block_result['status'] == 'failed':
                player1.hp -= dmg
                return _r(f"{player1.name} tried to defend against {player2.name}'s attack but failed. "
                        f"Attack hit and dealt {dmg} dmg")

            if block_result['status'] == 'fullsuccess':
                return _r(f"{player1.name} used his hammer to block the incoming attack taking no dmg at all while gaining {block_result['defense_buff']} defense")

            return _r(f"{player2.name} tried to attack {player1.name} but {player1.name} defended most of the dmg and "
        ...
        if player1.current_stance == 'block' and player2.current_stance == 'block':
            player1.hp -= 7
            player2.hp -= 7
            return _r(f"Both {player1.name} and {player2.name} chose to block. Both Lost 7 health ")

        # Player1 blocks, Player2 counters
        if player1.current_stance == 'block' and player2.current_stance == 'counter':
            player1.hp -= 2
            player2.speed -= 4
            return _r(f"{player1.name} blocked while {player2.name} tried to counter. No damage dealt. Stats debuffed -2hp for p1 and -4 speed for player 2")
        ...
        if player1.current_stance == 'counter' and player2.current_stance == 'attack':
            ...
            return _r(f"{player2.name} attacked but {player1.name} countered dealing {counter_dmg} damage back.")

        # Player1 counters, Player2 blocks
        if player1.current_stance == 'counter' and player2.current_stance == 'block':
            player1.speed -= 4
            player2.hp -=2
            return _r(f"{player1.name} tried to counter but {player2.name} blocked. No damage dealt. Stats lowered for both party. Loosing 4 speed and 2 hp respectively")
        ...
        if player1.current_stance == 'counter' and player2.current_stance == 'counter':
            ...
            return _r(f"Both {player1.name} and {player2.name} tried to counter. No damage dealt. - 10 defense for both")

        # Player1 recovers, Player2 attacks
        if player1.current_stance == 'recover' and player2.current_stance == 'attack':
            dmg = player2.deal_dmg(player1)
            player1.regen(player2)
            return _r(f"{player1.name} tried to recover but was interrupted by {player2.name}'s attack dealing {dmg} damage.")
        ...
        if player1.current_stance == 'recover' and player2.current_stance == 'block':
            ...
            if regen_result['status'] == 'blocked':
                ...
                return _r(f"Nuh uh {player1.name} no healing allowed this match.{player2.name} lost {block_result['defense_debuff']} defense")
            ...
            if block_result['status'] == 'wrong_guess':
                return _r(f"{player1.name} successfully got his {recovered_stat} by {recovered_amount} points while {player2.name} blocked loosing {block_result['defense_debuff']} defense.")
        ...
        if player1.current_stance == 'block' and player2.current_stance == 'recover':
            ...
            if regen_result['status'] == 'blocked':
                ...
                return _r(f"Nuh uh {player2.name} no healing allowed this match.{player1.name} lost {block_result['defense_debuff']} defense")
            ...
            if block_result['status'] == 'wrong_guess':
                return _r(f"{player2.name} successfully got his {recovered_stat} by {recovered_amount} points while {player1.name} blocked loosing {block_result['defense_debuff']} defense.")
        ...
        if player1.current_stance == 'attack' and player2.current_stance == 'recover':
            ...
            dmg = player1.deal_dmg(player2)
            return _r(f"{player2.name} tried to recover but was interrupted by {player1.name}'s attack dealing {dmg} damage.")
        ...
        if player1.current_stance == 'counter' and player2.current_stance == 'recover':
            ...
            if regen_result['status'] == 'blocked':
                ...
                return _r(f"Nuh uh {player2.name} no healing allowed this match. {player1.name} lost hp defense and speed by {counter_result['hp_drain']}, {counter_result['defense_drain']}, {counter_result['speed_drain']}")
        ...
        if player1.current_stance == 'recover' and player2.current_stance == 'recover':
            return _r(f"Both {player1.name} and {player2.name} tried to recover but failed")
        ...
        if player1.current_stance == 'cast' and player2.current_stance == 'cast':
            if player1.speed == player2.speed:
                player1.mana -= 5
                player2.mana -= 5
                return _r("same speed both intrupped each others spell. unsucsseful cast drains 5 mana for each")

            if player1.speed > player2.speed:
                ok, msg = player1.cast(player2)
                if not ok:
                    player1.hp -= 15
                    return _r(f"{player1.name} tried casting without enough mana took drained his own life in carelessness -15 HP")

                player2.mana -= 5
                return _r(f"Due to higher speed {player1.name} was able to interupt {player2.name} and cast...\n {player2.name} also lost 5 mana for failed attempt to cast \n {msg}")

            if player2.speed > player1.speed:
                ok, msg = player2.cast(player1)
                if not ok:
                    player2.hp -= 15
                    return _r(f"{player2.name} tried casting without enough mana took drained his own life in carelessness -15 HP")
                player1.mana -= 5
                return _r(f"Due to higher speed {player2.name} was able to interupt {player1.name} and cast...\n {player1.name} also lost 5 mana for failed attempt to cast \n {msg}")
        ...
        return _r("No valid moves resolved this round.")
