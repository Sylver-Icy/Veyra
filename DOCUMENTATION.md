...
- Weapon: Dark Blade (Attack +8, disables healing)
- Spell: Fireball (16 damage)
- AI randomly chooses moves (40% attack, 20% each for block/counter/recover)

**Note:** Campaign currently has no rewards (inferred from code analysis).
...
The following behaviors are inferred from code analysis but not explicitly documented:

1. **Campaign rewards:** Currently no rewards for defeating Veyra in campaign mode
2. **Item durability:** The `item_durability` field exists but appears unused
3. **Lottery system:** Lottery entries table exists but full implementation unclear
4. **Win streak tracking:** `battle_loadout.win_streak` column exists but usage not found
5. **Quest limits:** `quests.limit` column increments but enforcement unclear
...
*This documentation was generated by analyzing the complete Veyra codebase. For the most accurate and up-to-date information, always refer to the source code.*
