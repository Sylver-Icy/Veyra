...
| **AI Chat** | Mention-triggered conversation hook backed by an external service adapter (WIP) |
...
1. Create a `veyra.env` file with the following:
   ...
   DB_URL=postgresql+psycopg2://user:password@host:5432/veyra

   # Optional
   TEST_DB_URL=postgresql+psycopg2://user:password@host:5432/veyra_test
   CONVO_MODEL_URL=http://127.0.0.1:8000/chat
   CONVO_MODEL_API_KEY=your_conversation_service_key
   ```
...
|-----------|---------|-------------|
| Discord bot | Commands, events, tutorial, scheduler bootstrap | `python veyra.py` |
| REST API | Read-only profile/inventory/economy endpoints + transfer route | `uvicorn api.main:app --reload` |
| Database bootstrap | Creates ORM-managed tables if missing | `python database/dbsetup.py` |
| Conversation service | Optional external chat backend for mention replies | `CONVO_MODEL_URL` + `CONVO_MODEL_API_KEY` |

```mermaid
flowchart LR
    A["Discord Server"] --> B["Veyra Bot (veyra.py)"]
    B --> C["PostgreSQL"]
    B --> D["APScheduler Jobs"]
    B --> E["Optional Conversation Service"]
    F["FastAPI (api/main.py)"] --> C
```
...
3. Use `/help` or `/commandhelp <command_name>` for assistance
...
- **Transfers:** Gold sent to other players or donated to Veyra
...
|---------|------------------|
| Discord command (`/transfer_gold`) | Sends 95% of the entered amount to the recipient and uses that same net amount for the wallet mutation path |
| REST API (`POST /economy/transfer`) | Uses the economy transfer service, which applies the transfer fee to the raw amount and returns fee-aware transfer details |
...
**Starter Loan:**
- One-time loan currently exposed through the Discord UI
- Amount: 2,000 gold
...
**Additional Loan Packs:**

| Status | Notes |
|--------|-------|
| Defined in code | Multiple higher-tier loan packs are declared in `domain/loans/rules.py` |
| Exposed in Discord today | Only the starter pack is currently wired into the slash-command flow |
...
| 15 | 1× Potion of Hatred |

**Campaign Completion:**
- Reaching campaign stage 10 unlocks **Veyra's Grimoire** and **Veil of Darkness** in the loadout rules
- Reaching campaign stage 15 unlocks **Bardok's Claymore** in the loadout rules
- Once Stage 15 is cleared, your stored stage advances to 16 and `/campaign` shows the completion message
- `Earthquake` is currently available in the spell rules without an extra campaign-stage gate
...
⭐ *Unlocked once your campaign stage reaches 10*
⭐⭐ *Unlocked once your campaign stage reaches 15*
...
*Note: `Earthquake` appears in later campaign stages, but the current loadout rules do not place an extra stage lock on it.*
...
| `/commandhelp <command_name>` | Show detailed help for one command | 5s |
...
| `/transfer_gold @user <amount>` | Send gold (recipient currently receives 95% of entered amount) | - |
...
| Router | Prefix | Backing Module |
|--------|--------|----------------|
| Economy | `/economy` | `api/routes/economy.py` |
| Inventory | `/inventory` | `api/routes/inventory.py` |
| Profile | `/profile` | `api/routes/profile.py` |

### Economy Endpoints

| Method | Endpoint | Response Model | Description |
|--------|----------|----------------|-------------|
| `GET` | `/economy/balance/{user_id}` | `BalanceResponse` | Get user gold balance |
| `POST` | `/economy/transfer` | `TransferGoldResponse` | Transfer gold between users through the economy service |

`GET /economy/balance/{user_id}` response:
...
`POST /economy/transfer` response:
...
*Note: The API route delegates to `services.economy_services.transfer_gold()`, which applies the fee-aware transfer rules from the economy domain layer.*

### Inventory Endpoints

| Method | Endpoint | Response Model | Description |
|--------|----------|----------------|-------------|
| `GET` | `/inventory/{user_id}` | `InventoryResponse` | Get inventory snapshot with item details |

`GET /inventory/{user_id}` response shape:

| Field | Type | Notes |
|-------|------|-------|
| `user_id` | `int` | Target user ID |
| `items` | `InventoryItemSchema[]` | Each item includes `item_id`, `name`, `rarity`, `quantity`, and optional `description` |

### Profile Endpoints

| Method | Endpoint | Response Model | Description |
|--------|----------|----------------|-------------|
| `GET` | `/profile/{user_id}/energy` | `EnergyResponse` | Get current energy state |
| `GET` | `/profile/{user_id}/friendship` | `FriendshipResponse` | Get friendship title + progress |
| `GET` | `/profile/{user_id}` | `DashboardProfileResponse` | Get dashboard profile payload |

`GET /profile/{user_id}` currently returns:

| Field | Type | Notes |
|-------|------|-------|
| `user_name` | `str` | Stored Veyra username |
| `exp` | `int` | Current EXP |
| `level` | `int` | Current level |
| `gold` | `int` | Wallet gold |
| `friendship_exp` | `int \| null` | Friendship EXP if row exists |
| `loadout` | `object \| null` | Weapon, spell, and win streak when available |
| `inventory_preview` | `array` | First few inventory entries for dashboard display |
...
│   ├── game_events_services.py   # Recent event timeline for chat/profile context
│   ├── talk_to_veyra/            # AI chat service adapters
│   │   ├── chat_services.py      # Conversation API client
│   │   └── user_builder.py       # User payload builder for chat requests
...
- `game_events` - Recent gameplay event log used for profile/chat context
- `battle_queue` - Auto-match queue entries with min/max bet ranges
...
`veyra.py` also runs a small bootstrap path on startup before the recurring jobs settle in.

| Step | What Happens |
|------|--------------|
| Schema init | `database/sessionmaker.py` ensures ORM tables exist |
| Command sync | Slash commands are synced to Discord |
| Invite cache | Referral/invite cache is built |
| Daily seed | Shop tables are populated if needed |
| Lottery seed | A lottery post is sent during startup bootstrap |
| Scheduler start | Recurring APScheduler jobs are registered and started |
| Chat health check | The conversation service is pinged through its health endpoint |
...
3. Update `advance_campaign_stage()` cap if extending beyond the current 15-stage campaign
...
```mermaid
flowchart LR
    A["Jobs / Quests / Lootboxes"] --> B["Wallet (Gold)"]
    B --> C["Shop Purchases"]
    B --> D["Marketplace Trades"]
    B --> E["PvP Bets"]
    B --> F["Race Bets"]
    B --> G["Buildings / Smelting"]
```
...
flowchart LR
    ...
    C --> D["Bars"]
    D --> E["Sell / Use / Trade"]
    F["Gold"] --> G["Unlock or Upgrade Smelter"]
    G --> C
```
...
```mermaid
flowchart TD
    P1["Player Loadout"] --> BM["Battle Manager"]
    P2["Opponent Loadout"] --> BM
    BM --> R1["Resolve stances"]
    BM --> R2["Apply damage / mana / effects"]
    BM --> R3["Advance round or end battle"]
```
...
```mermaid
flowchart LR
    S1["Stages 1-10: Veyra"] --> S2["Stage 10 reward / unlock point"]
    S2 --> S3["Stages 11-15: Bardok"]
    S3 --> S4["Stage 16: campaign complete state"]
```
...
1. **Check status:** `!helloVeyra` to register if needed or review your friendship title
...
flowchart LR
    ...
    B --> C["Jobs / Quests"]
    C --> D["Shop / Marketplace"]
    C --> E["Lootboxes / Items"]
    E --> F["Mining / Crafting"]
    F --> G["Building Upgrades"]
    G --> H["PvP / Campaign"]
```
...
7. Reach Stage 15 to unlock Bardok's Claymore in the loadout rules
8. Clear Stage 15 to move into the campaign-complete state
9. Use your campaign unlocks in PvP battles for an edge
